...
import { icxGetScoreFromRPC, icxCall, icxSendTransactionRaw } from '../../../redux/api/restV3/icx'
...
function ContractExplorerPage({ wallet, url, walletType, bip44Path }) {
    ...
    const [inputItemsState, setInputItemsState] = useState(getInitialInputState(url))
    ...
    async function handleClickOnWrite(address, method, inputs, index, networkState, endpoint) {
        if (wallet === '') {
            ...
            if (walletType === 'ICONEX') {
                ...
                txResult.valueArray = response.data == null ? '' : [response.data.result]
                txResult.state = 1
                ...
                try {
                    const signedTx = await ledger.signTransaction(serializedTx, bip44Path)
    ...
    useEffect(() => {
        ...
        if (isValidCxAddress) {
            if (networkState === 'custom') {
                if (endpointInputHasFocus === false && isValidUrl(inputItemsState.endpoint)) {
        ...
        <div className={styles.main}>
            <div className={styles.mainContainer}>
                ...
                <div className={`${styles.pageContent}`}>
                    <div className={`${styles.pageContentHeader} ${styles.paperContainer}`}>
                        <InputItem
                            label={`Address`}
                    ...
                    <div className={`${styles.pageContentBody} ${styles.paperContainer}`}>
                        ...
                        <div className={styles.contractContainer}>
                            {activeSection === 0 ? (
                                ...
                                <div className={styles.titleContainer}>
                                    <span className={styles.titleItem}>Read Contract Methods</span>
                                    {contractAbi == null ? (
                                        <LoadingComponent height="322px" />
                                    ) : contractAbi.error != null ? (
                                        <div className="scroll">
                                ...
                                <div className={styles.titleContainer}>
                                    <span className={styles.titleItem}>Write Contract Methods</span>
                                    {contractAbi == null ? (
                                        <LoadingComponent height="322px" />
                                    ) : contractAbi.error != null ? (
                                        <div className="scroll">
            ...
            <GenericModal isOpen={ledgerWaitModalState} onClose={closeLedgerModal} useSmall={true}>
...
function InputItem({
    ...
    return (
        <div
            className={
                halfSize ? `${styles.inputItem} ${styles.inputItemHalfSize}` : `${styles.inputItem}`
            }>
...
function Separator({ useVertical = false }) {
    return (
        <div className={useVertical ? `${styles.separatorVertical}` : `${styles.separator}`}></div>
    )
}
