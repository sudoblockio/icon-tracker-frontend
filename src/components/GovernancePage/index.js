                        ...
                        <div className="wrap-holder">
                            <div className="contents">
                                ...
                                <p className="prep-causion">
                                    This data is periodically synced. Changes can take up to 10
                                    minutes to show up.
                                </p>
                            </div>
                        </div>
...
class TableRow extends Component {
    ...
    getJailBadge = (jail_flag) => {
        ...
        return (
            <div>
                <span className={`jail-badge ${badgeClass}`}>{badgeText}</span>
            </div>
        )
    }
    goAddress = (address) => {
    ...
    render() {
        ...
        const rank = index + 1
        const bondedRate = !totalVoted
            ? 0
            : (20 * (bonded / Math.pow(10, 18)).toFixed()) / prepVoted
        ...
        return (
            <tr>
                ...
                <td>
                    {cps_governance
                        ? sponsored_cps_grants === 0 || sponsored_cps_grants === null
                            ? '✓'
                            : sponsored_cps_grants
                        : '-'}
                </td>
                ...
                <td>
                    ...
                    isNaN(max_commission_change_rate) &&
                    isNaN(max_commission_rate) ? (
                        '-'
                    ) : (
                        <>
                            <em>
                                <span>
                                    {isNaN(commission_rate)
                                        ? '-'
                                        : numberWithCommas(Number(commission_rate).toFixed())}
                                    %
                                </span>
                            </em>
                            <br />
                            <em>
                                {isNaN(max_commission_change_rate)
                                    ? '-'
                                    : numberWithCommas(
                                          Number(max_commission_change_rate).toFixed()
                                      )}
                                /{' '}
                                {isNaN(max_commission_rate)
                                    ? '-'
                                    : numberWithCommas(Number(max_commission_rate).toFixed())}
                            </em>
                        </>
                    )}
                </td>
