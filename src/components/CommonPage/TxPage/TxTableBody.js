...
const TxAddressCell = ({ isError, address }) => {
    ...
    if (!isValidData(address)) {
        ...
        _address = <AddressLink to={address} label={<span className="ellipsis">{address}</span>} />
...
const TxHashCell = ({ isError, txHash }) => {
    ...
    if (!isValidData(txHash)) {
        ...
        _txHash = _is0xHash ? (
            <TransactionLink to={txHash} label={<span className="ellipsis">{txHash}</span>} />
        ) : (
            txHash
        )
...
const TokenCell = ({ name, address }) => {
    return (
        <td>
            <span className="ellipsis">{tokenText(name, undefined, address, 'ellipsis')}</span>
        </td>
    )
}
...
const DateCell = ({ date, isDate, type }) => {
    ...
    return (
        <td className={className}>
            {type === 'Age' ? dateText : type === 'Date Time (UTC)' ? changedDate : dateText}
        </td>
    )
}
                ...
                case TX_TYPE.ADDRESS_TX:
                    return (
                        <tr>
                            ...
                            <AddressSet
                                fromAddr={data.from_address !== 'None' ? data.from_address : '-'}
                ...
                case TX_TYPE.TRANSACTIONS:
                    return (
                        <tr>
                            ...
                            <AmountCell
                                amount={
                                    convertHexToValue(data.value) || convertHexToValue(data.amount)
                                }
                                symbol="ICX"
                            />
