...
class RecentBlocks extends Component {
  ...
  async componentDidMount() {
    ...
    this.bxsocket.onmessage = async (event) => {
      ...
      this.setState((prev) => {
        const newArray = [{ ...JSON.parse(event.data), isNew: true }].concat(prev.recentBx).slice(0, 8);
  ...
  handleKeyDown = (e) => {
    if (e.key === "i") {
      ...
      if (this.state.play === false) {
        ...
        console.log(this.bxsocket, "bsxsocket");
        this.bxsocket.onmessage = async (event) => {
          this.latestBx = event.data;
          this.setState({ liveTrClass: "flat" });
          const blockListData = await blockList();
          ...
          {loading ? (
            ...
            <ul className={"list"} style={{ height: list.length === 0 ? 511 : "" }}>
              ...
              {list.map((block, index) => {
                ...
                return (
                  <li key={index} className={block.isNew ? `fade-in-anim` : ""}>
                    <p className="icon">B</p>
                    <p className="a">
                      Block
                      <em>
                        <BlockLink
                          ...
                          label={numberWithCommas(number) || numberWithCommas(latest.number - 1)}
                        />
                      </em>
                    </p>
                    <p className="b">
