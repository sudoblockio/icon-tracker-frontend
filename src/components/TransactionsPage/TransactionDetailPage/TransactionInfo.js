...
import { CopyButton, LoadingComponent, AddressLink, BlockLink, AddressCell, ReportButton } from "../../../components";
import {
  makeDownloadLink,
  convertNumberToText,
  numberWithCommas,
...
class TransactionInfo extends Component {
  ...
  render() {
    ...
    const toUSDNum = Number(
      this.state.currentUSD * convertHexToValue(this.props.transaction.data.transaction_fee)
    ).toFixed(4);
    const Contents = () => {
      if (loading) {
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td className={scam ? "scam" : ""}>
                          ...
                          <CopyButton data={hash} title={"Copy TxHash"} isSpan />
                      ...
                      <tr>
                        ...
                        <td
                          ...
                          onClick={() => isFail && window.open(`/transaction/trace/${hash}`)}
                          className={isFail ? "fail" : ""}
                        >
                          {" "}
                          {isSuccess ? "Success" : "Fail"} {isFail && `- ${this.state.msgList}`}
                          <span
                            className={"btn-scam"}
                            onClick={() => window.open(`/transaction/trace/${hash}`)}
                          >
                            View Logs
                          </span>
                        </td>
                      ...
                      <tr>
                        ...
                        <td>
                          <span>
                            <BlockLink to={block_number} label={numberWithCommas(block_number)} />
                          </span>
                          <em>{`(${
                            this.state.lastBlock ? Number(this.state.lastBlock.height - block_number) : " -"
                          } Confirmation(s))`}</em>
                        </td>
                      </tr>
                      <tr>
                        <td>Time Stamp</td>
                        <td>
                          {new Date(block_timestamp / 1000).toString()}
                          <em>{epochToFromNow(block_timestamp)}</em>
                        </td>
                      </tr>
                      <tr>
                        <td>From</td>
                        <AddressRow
                          address={data.from_address !== "None" ? data.from_address : "-"}
                          txType={txType}
                      ...
                      <tr>
                        <td>To</td>
                        <AddressRow
                          address={data.to_address !== "None" ? data.to_address : "-"}
                          internalTxList={(transactionInternalTx && transactionInternalTx.data) || []}
                          txType={txType}
                          targetContractAddr={targetContractAddr}
                          download={download}
                        />
                      ...
                      <tr>
                        <td>Step Price</td>
                        <td>
                          {convertNumberToText(stepPriceIcx)} ICX<em>({convertNumberToText(stepPriceGloop)} Gloop)</em>
                        </td>
                      </tr>
                      <tr>
                        <td>Step Limit</td>
                      ...
                      <tr>
                        <td>Fee in ICX</td>
                        <td>
                          {convertHexToValue(transaction_fee)} ICX<em>({transaction_fee ? toUSDNum : " -"} USD)</em>
                        </td>
                      </tr>
                      ...
                      {data_type && dataString ? (
                        <tr>
                          ...
                          <DataCell
                            ...
                            imageConverterPopup={this.props.imageConverterPopup}
                          />
                        </tr>
                      ) : null}
                    </tbody>
  ...
  render() {
    ...
    return (
      <td className="convert">
        ...
        {isButton && (
          <button className="btn-type-normal" onClick={this.handleClick} disabled={scam}>
            {buttonTitle}
          </button>
        )}
      </td>
    ...
    return (
      <td className="transfer">
        {tokenTxListSliced.map((tokenTx, index) => {
          ...
          return (
            <p key={index}>
              {value_decimal} {token_contract_symbol}
              <em>({token_contract_name})</em>
              &emsp;from &emsp;
              <AddressLink to={from_address} label={<span className="ellipsis">{from_address}</span>} />
              &emsp;to&emsp;
              <AddressLink to={to_address} label={<span className="ellipsis">{to_address}</span>} />
            </p>
          );
        })}
        {isMoreTen && (
          <span className={`more`} onClick={this.toggleMore}>
            More ({more ? length : length - COUNT} / {length})<i className={`img${more ? " on" : ""}`}></i>
          </span>
        )}
      </td>
    );
    ...
    return (
      <div>
        {internalTxListSliced.map((tx, index) => {
          ...
          return (
            <p key={index}>
              ┗&emsp;TRANSFER {convertHexToValue(value).toFixed(4)} ICX &emsp;from &emsp;
              <span>
                <AddressLink to={from_address} />
              </span>
              &emsp;to&emsp;
              <span>
                <AddressLink to={to_address} />
              </span>
            </p>
          );
        })}
