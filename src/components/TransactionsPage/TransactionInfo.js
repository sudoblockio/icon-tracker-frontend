...
const AddressCell = ({ address, internalTxList }) => {
	...
	if (isAddress) {
		...
		return (
			<td className="trans">{isContract && <i className="img"></i>}<span><WalletLink to={address} /></span><CopyButton data={address} title={'Copy Address'} isSpan />
				{
					isInternalTxList &&
					<div>
						{internalTxList.map((tx, index) => {
							const { amount, fromAddr, toAddr } = tx
							return <p key={index}>┗&emsp;TRANSFER {convertNumberToText(amount, 'icx')} ICX&emsp; from &emsp;<span><WalletLink to={fromAddr} /></span>&emsp;to&emsp;<span><WalletLink to={toAddr} /></span></p>
						})}
					</div>
				}
			</td>
