...
export function Hr() {
    return <div style={{ height: '2px', width: '100%', backgroundColor: '#d6d3d1' }}></div>
}
    ...
    return (
        <div className={styles.imgLoading}>
            {[1, 2, 3, 4, 5].map((foo, index) => (
                <div className={styles.imgLoadingItem} key={`loading-component-${index}`}></div>
            ))}
        </div>
    )
}

export function WalletResponseModal({ isOpen, onClose, txData, walletResponse }) {
    // useEffect(() => {
