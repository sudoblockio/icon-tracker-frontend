...
export async function contractTokenTxList(payload) {
  ...
  const trackerApi = await trackerApiInstance()
  return new Promise((resolve, reject) => {
    trackerApi.get(makeUrl(`${TRANSACTIONS_PREFIX}/token-transfers/token-contract/${payload.addr}`, payload))
      .then(result => {
        resolve(result)
      })
      .catch(error => {
...
export async function contractInternalTxList(payload) {
  const trackerApi = await trackerApiInstance()
  return new Promise((resolve, reject) => {
    trackerApi.get(makeUrl(`${TRANSACTIONS_PREFIX}/internal/address/${payload.addr}`, payload))
      .then(result => {
        resolve(result)
      })
      .catch(error => {
...
export function contractsReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case actionTypes.contractListSearch:
      return getState('ARR', REDUX_STEP.READY, state, action, 'contractsSearch')
    case actionTypes.contractListSearchFulfilled:
      return getState('ARR', REDUX_STEP.FULFILLED, state, action, 'contractsSearch')
    case actionTypes.contractListSearchRejected:
      return getState('ARR', REDUX_STEP.REJECTED, state, action, 'contractsSearch')
    ...
    case actionTypes.contractDetailFulfilled:
      return getState('OBJ', REDUX_STEP.FULFILLED, state, action, 'contractDetail')
    case actionTypes.contractDetailRejected:
      return getState('OBJ', REDUX_STEP.REJECTED, state, action, 'contractDetail')
    ...
    case actionTypes.contractInternalTxList:
      return getState('ARR', REDUX_STEP.READY, state, action, 'contractInternalTx')
    case actionTypes.contractInternalTxListFulfilled:
      return getState('ARR', REDUX_STEP.FULFILLED, state, action, 'contractInternalTx')
    case actionTypes.contractInternalTxListRejected:
      return getState('ARR', REDUX_STEP.REJECTED, state, action, 'contractInternalTx')

    case actionTypes.contractTokenTxList:
      return getState('ARR', REDUX_STEP.READY, state, action, 'contractTokenTx')
    case actionTypes.contractTokenTxListFulfilled:
      return getState('ARR', REDUX_STEP.FULFILLED, state, action, 'contractTokenTx')
    case actionTypes.contractTokenTxListRejected:
      return getState('ARR', REDUX_STEP.REJECTED, state, action, 'contractTokenTx')
    ...
    case actionTypes.contractEventLogListFulfilled:
      return getState('ARR', REDUX_STEP.FULFILLED, state, action, 'contractEvents')
    case actionTypes.contractEventLogListRejected:
      return getState('ARR', REDUX_STEP.REJECTED, state, action, 'contractEvents')
    ...
    case actionTypes.icxGetScoreFulfilled:
      return getState('OBJ', REDUX_STEP.FULFILLED, state, action, 'contractAbi')
